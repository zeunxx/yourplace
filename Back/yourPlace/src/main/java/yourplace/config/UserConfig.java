	...
	public void configure(HttpSecurity http) throws Exception{
		...
		http
			...
			.authorizeRequests() // andMatchers() 사용하면 url/http 메소드 별로 관리 가능
					.antMatchers("/login","/signup","/").permitAll() // 로그인,회원가입,메인 페이지 아무나 접근 가능
					.antMatchers("/user").hasRole("USER") // USER(유저), ADMIN(관리자)만 접근가능
			...
			.and()
				.formLogin() // 로그인 설정
					...
					.defaultSuccessUrl("/mypage") // 로그인 성공시 연결되는 주소(메인페이지로 이동)
	...
	public void configure(AuthenticationManagerBuilder auth) throws Exception{
		...
		auth.userDetailsService(userServiceImpl) //유저 정보는 userService에서 가져옴
			.passwordEncoder(new BCryptPasswordEncoder()); //패스워드 인코더 설정
							// (+ 제출된 비밀번호와 DB에 있는 비밀버노 일치 여부 알려주는 메소드 제공)
	}
